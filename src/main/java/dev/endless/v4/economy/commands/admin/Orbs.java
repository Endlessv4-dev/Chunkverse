...
public class Orbs implements TabExecutor {
    ...
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command command, @NotNull String label, @NotNull String[] args) {
        ...
        if (!sender.hasPermission("chunkverse.orbs." + action)) {
        ...
        if (!action.equals("reset")) {
            if (args.length < 3) {
                sender.sendMessage("§c§lFAILED | §7Usage: /orbsa " + action + " <player> <amount>");
                return true;
            }
            try {
    ...
    private void sendVelocityUpdate(CommandSender sender, Player target, String action, double amount) {
        ...
        sender.sendMessage("§a§lSUCCESS | §7Orbs request sent to Proxy.");
    }
    ...
    public @Nullable List<String> onTabComplete(@NotNull CommandSender sender, @NotNull Command command, @NotNull String alias, @NotNull String[] args) {
        ...
        if (args.length == 1) {
            ...
            for (String s : subCommands) {
                if (sender.hasPermission("chunkverse.orbs." + s)) completions.add(s);
            }
